...
Say *"capture my voice"*. Idiolect builds the profile from writing you point it at, or through its guided interview when you have no samples, and it refines the profile every time you react to a piece ("I'd never say that"). A profile can also live user-wide in `~/.idiolect/profiles/<name>/`, shared by every project. The wiki itself is private working text, so its pages are never scanned. When you say a piece is going to a wider audience, or ask for a scan, it goes through the bundled hogwash skill on its own defaults; the wiki writes no `hogwash.json`. Hogwash's rule packs are the real ban list (the machine-writing tells); the idiolect profile adds your own bans and voice, and hogwash's rewrite loop applies it from its default `profile/` path, so one symlink (`~/.idiolect/profile -> ~/.idiolect/profiles/<name>`) makes your voice the default in every project.
