...
5. Before calling it done, scan it with hogwash: `bun .claude/skills/hogwash/scripts/hogwash.ts scan --register prose --fail-on error <draft.md>`. Hogwash runs on its defaults (this wiki has no `hogwash.json`): its packs are the real ban list, and it applies the owner's idiolect profile (their added bans and voice) from its default `profile/` path, in the wiki or `~/.idiolect/profile/` (a symlink to `~/.idiolect/profiles/<name>`; `doctor.ts` checks). Everything voice goes through hogwash; do not run a parallel check. A deck or doc spec is JSON, so run `bun scripts/spec-prose.ts <spec.json>` first and scan the `.prose.md` it writes. Then idiolect's ordered self-check (substance, bans, register, mechanics, rhythm, attribution).
