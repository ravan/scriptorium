...
5. Before calling it done, scan it with hogwash: `bun .claude/skills/hogwash/scripts/hogwash.ts scan --fail-on error <draft.md>` (machine-writing rules, the ban list, and the house mechanics this wiki turns on in `hogwash.json`: connector dashes, sentences over one comma, paragraphs past three sentences). A deck or doc spec is JSON, so run `bun scripts/spec-prose.ts <spec.json>` first and scan the `.prose.md` it writes. Then idiolect's ordered self-check (substance, bans, register, mechanics, rhythm, attribution).
