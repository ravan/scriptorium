...
Between them these settle checks 1, 4 and 7 mechanically, so your attention goes to the judgement checks below. `links.ts` reports broken relative links, `[[wikilinks]]`, manifest entries pointing at pages that no longer exist, sources marked ingested with no source page, orphans, and pages missing their purpose line. It exits 1 when anything is actually broken; orphans and missing purpose lines are reported but do not fail. It also regenerates `wiki/index.json` and `wiki/map.json`, so the catalog and graph are current by the time you look - `bun scripts/wiki.ts hubs` and `clusters` are a good starting point for checks 5 and 6. Check 7 (index drift) no longer exists: the index is generated from the pages.
